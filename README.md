...
- Optional spatial landing-site cone with an adjustable 15°–90° half-angle, applied to every predicted position;
- Randomized initial positions are generated inside the selected cone whenever the constraint is enabled, preserving initial OCP feasibility;
...
6. Handles the optional all-horizon landing-cone inequalities with augmented-Lagrangian iterations and weighted half-space projection;
7. Uses backtracking line search on the nonlinear merit function;
8. Applies the first input and shifts the solution to warm-start the next sample. Changing the landing-cone mode discards the old trajectory warm start so a solution from the previous feasible set cannot bias the first re-solve.

The diagnostics report the number of cone inequalities actually passed to the QP: `0` when disabled and `2N` when enabled. If the mode is changed after a completed landing or during replay, the demo returns to the same initial state because the recorded closed-loop trajectory belongs to the previous OCP.
...
Result: 10.11 s soft landing, 0.06 m position error,
        0.08 m/s touchdown speed, 0.2 deg tilt
```
...
- `DYNAMIC_MODEL.md`: standalone state, input, and nonlinear dynamic-model definition;
